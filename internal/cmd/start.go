...
func runStartCrew(cmd *cobra.Command, args []string) error {
	...
	ensureDefaultBranch(worker.ClonePath, fmt.Sprintf("Crew workspace %s/%s", rigName, name))
...
func startCrewMember(rigName, crewName, townRoot string) error {
	...
	// Ensure crew workspace is on default branch
	ensureDefaultBranch(worker.ClonePath, fmt.Sprintf("Crew workspace %s/%s", rigName, crewName))
